...
function Microphone() {
    ...
    this.initialize = function() {
        ...
        fft = null;
        spectrum = null;
        myFFTSize = 2048;   // Don't change myFFTSize or sampleRate unless you
        sampleRate = 44100; // have a VERY good reason
        freqRes = (sampleRate/2)/(myFFTSize/2);
        processing = false;
        notes = {"A0" : 27.5, "A#0" : 29.1352, "B0" : 30.8677, "C1" : 32.7032, 
        	...
        	"A1" : 55, "A#1" : 58.2705, "B1" : 61.7354, "C2" : 65.4064, 
    ...
    function computeFrequency() {
        ...
        return Math.round(interpolatedBin*freqRes);
    }

    // -------------------------------------------------------------------------
    // jainsMethodInterpolate function.
    // Uses neighbouring indices to the index of greatest magnitude to
    // create a more accurate estimate of the frequency.
    ...
    this.getCurrentFrequency = function() {
        if (!processing) {
            throw "Can't get frequency. Mic not listening";
        }
        else {
    ...
    function matchNote(freq) {
        ...
        for (var key in notes) { // Iterates through note look-up table
        		// If the current note in the table is closer to the given frequency
        		// than the current "closest" note, replace the "closest" note.
    ...
    this.getCurrentNote = function() {
        var currFreq = this.getCurrentFrequency();
        var noteInfo = matchNote(currFreq);
        return noteInfo[0];
    }

    // -------------------------------------------------------------------------
    // getNoteAndCentsOffset function.

    this.getCurrentNoteAndCentsOffset = function() {
        var currFreq = this.getCurrentFrequency();
        var noteInfo = matchNote(currFreq);
